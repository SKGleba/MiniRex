...
int main() {
	...
	const char PSTVNotContinue[256] = "This installer can NOT continue.\n\nThis version is made for fw 3.60 & 3.61 ONLY\n\nPress X to exit...";
	const char MainMenuOpts[5][256] = {"Flash DEX Spoof", "Flash TOOL Spoof", "Flash ITU Spoof", "Revert changes", "Exit this app"};
	const char SubMenuOpts[5][256] = {"Deeper Spoof", "Flash RAW spoof", "Flash ux0:skg/mrex/custom/fat.bin", "UI colour", "Exit this app"};
	const char nobMenuOpts[4][256] = {"DEX", "ITU", "TOOL", "Back"};
	const char ColMenuOpts[5][256] = {"Red", "White", "Green", "Blue", "Back"};
	const char YesNo[2][4] = {"No", "Yes"};
	
	int config_exists = exists("vs0:tai/boot_config.txt");
	int UIScreen = 0;
	int selected = 0;
	int doStuff = 0;
	int slimbin = 0;
	...
	while(1) {
		...
		if (exists("ur0:temp/mrexsbl") == 1){col1 = 0; col2 = 0; col3 = 255;}
		if (exists("ur0:temp/mrexdbl") == 1){col1 = 92; col2 = 226; col3 = 92;}
		...
		switch(UIScreen) {
			...
			case 4: // Install Ongoing
				...
				if (slimbin == 15) filecopy("ux0:app/SKGM1NR3X/stuff/mbrod", "ur0:temp/mrexmbr");
				if (slimbin == 7) filecopy("ux0:app/SKGM1NR3X/stuff/mbrot", "ur0:temp/mrexmbr");
	...
	vita2d_set_clear_color(RGBA8(0x00, 0x00, 0x00, 0xFF));
		vita2d_clear_screen();
				...
				for (int i = 0; i < 4; i++) {
					if (i == selected) {
						vita2d_draw_rectangle(70, (80*i) + 85, 820, 70, RGBA8(col1,col2,col3,255));
						vita2d_pgf_draw_text(font, 85, (80*i) + 130, RGBA8(0,0,0,255), 1.5f, nobMenuOpts[i]);
					} else vita2d_pgf_draw_text(font, 85, (80*i) + 130, RGBA8(255,255,255,255), 1.5f, nobMenuOpts[i]);
				}
				break;
			case 37: 
				vita2d_pgf_draw_text(font, 215, 45, RGBA8(255,255,255,255), 1.5f, "Flash raw spoof");
	vita2d_set_clear_color(RGBA8(0x00, 0x00, 0x00, 0xFF));
		vita2d_clear_screen();
				if (selected < 0) selected = 0;
				if (selected > 3) selected = 3;
		...
		if (exists("ur0:temp/mrexsbl") == 1) sceIoRemove("ur0:temp/mrexsbl");
		...
		if (keys_down & SCE_CTRL_CROSS) {
			switch(UIScreen) {
				...
				case 27:
					...
					break;
				case 37:
					if (selected == 0) {slimbin = 15; UIScreen = 3;}
					if (selected == 1) {slimbin = 79; UIScreen = 3;}
					if (selected == 2) {slimbin = 7; UIScreen = 3;}
					if (selected == 3) UIScreen = 7;
					break;
				case 8:
					if (selected == 0) {filecopy("ur0:tai/config.txt", "ur0:temp/mrexred"); col1 = 226; col2 = 92; col3 = 92; UIScreen = 7;}
					if (selected == 1) {col1 = 255; col2 = 255; col3 = 255; UIScreen = 7;}
					if (selected == 2) {filecopy("ur0:tai/config.txt", "ur0:temp/mrexdbl"); col1 = 92; col2 = 226; col3 = 92; UIScreen = 7;}
					if (selected == 3) {filecopy("ur0:tai/config.txt", "ur0:temp/mrexsbl"); col1 = 0; col2 = 0; col3 = 255; UIScreen = 7;}
